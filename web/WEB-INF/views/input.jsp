...
<div class="container">
    ...
    <form class="form-horizontal">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-10">
                <label class="radio-inline">
                    <input type="radio" name="empGender" id="inlineRadio1" value="1" checked> 男
                </label>
                <label class="radio-inline">
        ...
        <div class="form-group">
            <button type="button" id="emp-save-btn" class="btn btn-primary btn-lg btn-block">提交</button>
        </div>
    </form>
</div>
...
<script type="text/javascript">
            ...
            success:function (result) {
                if (result.code == 100){
                    ...
                    if (result.extend.fieldErrors.empName != undefined){
                        show_validation_msg("#inputName","error",result.extend.fieldErrors.empName);
                    }
    ...
    function chkEmpNameIsRepeat() {
        var flag = true;
        $.ajax({
            url:"${App_Path}/chkEmpName",
            data:{"empName":$("#inputName").val()},
            type:"get",
            dataType:"json",
            success:function (result) {
                if (result.code == 100){
                    show_validation_msg("#inputName","success","姓名可用！");
                    flag = true;
                }else {
                    show_validation_msg("#inputName","error","姓名不可用！");
                    flag =  false;
                }
            }
        });
    ...
    $("#inputName").blur(function () {
        ...
        if($("#inputName").val() == "") {
            ctrl.addClass("has-error");
            ctrl.find("span.help-block").text("员工名不可为空！");
        }else if($regEmail.test($("#inputName").val()) == false) {
            ctrl.addClass("has-error");
            ctrl.find("span.help-block").text("员工名需为3-16位字母或数字！");
        }else {
            if (chkEmpNameIsRepeat()){
                ctrl.addClass("has-success");
                ctrl.find("span.help-block").text("员工名格式符合使用！");
            }else {
                ctrl.addClass("has-error");
                ctrl.find("span.help-block").text("该员工已存在！");
            }
        }
    });
